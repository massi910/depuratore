            ...
            <div class="container p-0">
                <div class="p-1">
                    <div class="btn-group btn-group-toggle" data-toggle="buttons">
                        <label class="btn btn-light" :class="{ active : !props.carditem.utenza.b_manual }" 
                            >
                            <input type="radio" name="options" id="option1" autocomplete="off" 
                                :checked="!props.carditem.b_manual"
                                @click="updateVal('b_manual', false)">Automatico
                        </label>
                        <label class="btn btn-light" :class="{ active : props.carditem.utenza.b_manual }">
                            <input type="radio" name="options" id="option2" autocomplete="off" 
                                :checked="props.carditem.utenza.b_manual"
                                @click="updateVal('b_manual', '1')">Manuale
                        </label>
                    </div>
                        ...
                        <div v-if="props.carditem.utenza.b_manual" class="">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" 
                                    :id="props.carditem.utenza.id" :name="props.carditem.utenza.name" 
                                    :checked="props.carditem.utenza.b_status" @click="updateStatus">
                                <label class="custom-control-label" 
                                    :for="props.carditem.utenza.id">Attivazione</label>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        updateVal(prop, val)
        {
            let currentVal = this.utenza[prop];
            this.utenza[prop] = val;
            if (currentVal != val)
                this.$emit('write');
        }
    }
